<?php
  /*
    20171216
    /localhost/2017/admin-cart/ms_date_sale_qty_fornonpv.php
...
//checkout
    ...
    $stock_sku_a = [];
    $stockupdate_query = tep_db_query("select ish.ik_stock_products_history_id, ish.ik_stock_products_id, ish.user_name, ish.previous_qty, ish.quantity, ish.latest_qty, ish.comments, ish.date_modified, ish.products_id, ish.products_attributes, ish.ik_stock_checkout_reasons_id, p.manufacturers_id, pd.products_name, p2c.categories_id, cd.categories_name, m.manufacturers_name from ik_stock_products_history ish left join products p on ish.products_id=p.products_id left join products_description pd on p.products_id=pd.products_id left join products_to_categories p2c on p.products_id=p2c.products_id left join categories_description cd on p2c.categories_id=cd.categories_id left join manufacturers m on p.manufacturers_id=m.manufacturers_id where ish.quantity!= 0 and ish.date_modified between '" . $date1 . " 00:00:00' and '" . $date2 . " 23:59:59' and ish.comments = '". $comments_type . "' and pd.language_id=1 and cd.language_id=1 " . " order by ish.ik_stock_checkout_reasons_id, user_name, concat(ish.products_id, ish.products_attributes), ish.comments");
    ...
    $specials_string = "category, name, att, qty" . "\r\n";
    ...
    write_to_file($specials_string, "check" . $comments_type . "_h-" . preg_replace("/-/", "", $date1) ."-". preg_replace("/-/", "", $date2) . ".csv");

    $specials_string = "brand, name, att, qty" . "\r\n";
    ...
    write_to_file($specials_string, "check" . $comments_type . "_ha-" . preg_replace("/-/", "", $date1) ."-". preg_replace("/-/", "", $date2) . ".csv");
...
//sale
    ...
    $raw1="select op.orders_products_id, op.orders_id, op.products_id, op.products_model, op.products_name, op.final_price, op.products_tax, op.products_quantity, op.products_stock_attributes, opa.products_options, opa.products_options_values, p2c.categories_id, p.manufacturers_id from " . TABLE_ORDERS_PRODUCTS . " op left join " . TABLE_ORDERS . " o on op.orders_id=o.orders_id left join " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " opa on op.orders_products_id = opa.orders_products_id left join " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c on op.products_id=p2c.products_id left join " . TABLE_PRODUCTS . " p on op.products_id = p.products_id where  o.date_purchased between '" . $date1 . " 00:00:00' and '" . $date2 . " 23:59:59'";
...
//OSCLogger::writeLog($haircare, OSCLogger::DEBUG);
    ...
    $specials_string = "category, name, att, qty, sale" . "\r\n";
    foreach ($hair as $k1 => $v1) {
        $cdesc = tep_db_fetch_array(tep_db_query("SELECT categories_name FROM categories_description WHERE categories_id = " . (int)$k1 . " and language_id=1"));
        //$cdesc['categories_name'] = preg_replace("/\//", "", $cdesc['categories_name']);
        $specials_string .= $cdesc['categories_name'] . ", , , , " . "\r\n";
        foreach ($v1 as $k2 => $v2) {
            $specials_string .= " ," . $v2['pname'] . "," . $v2['patt_pv'] . "," . $v2['qty'] . "," . tep_round($v2['sale'], 2) . "\r\n";
        }
    }
    write_to_file($specials_string, "sale_h-" . preg_replace("/-/", "", $date1) ."-". preg_replace("/-/", "", $date2) . ".csv");
    $specials_string = "brand, name, att, qty, sale" . "\r\n";
    foreach ($haircare as $k3 => $v3) {
        ...
        //$mdesc['manufacturers_name'] = preg_replace("/\//", "", $mdesc['manufacturers_name']);
        foreach ($v3 as $k4 => $v4) {
            $specials_string .= " ," . $v4['pname'] . "," . $v4['patt_pv'] . "," . $v4['qty'] . "," . tep_round($v4['sale'], 2) . "\r\n";
        }
    }
    write_to_file($specials_string, "sale_ha-" . preg_replace("/-/", "", $date1) ."-". preg_replace("/-/", "", $date2) . ".csv");
