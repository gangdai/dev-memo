...
//checkout
    ...
    $stock_sku_a = [];
    $stockupdate_query = tep_db_query("select ish.ik_stock_products_history_id, ish.ik_stock_products_id, ish.user_name, ish.previous_qty, ish.quantity, ish.latest_qty, ish.comments, ish.date_modified, ish.products_id, ish.products_attributes, ish.ik_stock_checkout_reasons_id, p.parent_id, p.manufacturers_id, pd.products_name, p2c.categories_id, cd.categories_name, m.manufacturers_name from ik_stock_products_history ish left join products p on ish.products_id=p.products_id left join products_description pd on p.products_id=pd.products_id left join products_to_categories p2c on p.products_id=p2c.products_id left join categories_description cd on p2c.categories_id=cd.categories_id left join manufacturers m on p.manufacturers_id=m.manufacturers_id where ish.quantity!= 0 and ish.date_modified between '" . $date1 . " 00:00:00' and '" . $date2 . " 23:59:59' and (ish.comments = 'out' or ish.comments = 'in') and pd.language_id=1 and cd.language_id=1 " . " order by ish.ik_stock_checkout_reasons_id, user_name, concat(ish.products_id, ish.products_attributes), ish.comments");
    while($stockupdate = tep_db_fetch_array($stockupdate_query)) {
        ...
        if ($stockupdate['comments'] == "out" ) {
            $_out = abs($stockupdate['quantity']);
            $_in = 0;
        } elseif ($stockupdate['comments'] == "in" ) {
            $_in = abs($stockupdate['quantity']);
            $_out = 0; 
        } else {
            continue;
        }
        ...
        if (tep_not_null($stockupdate['products_attributes']) && $stockupdate['products_attributes'] != '') {
            ...
            if (!isset($stock_sku_a[$stock_sku])) {
                $stock_sku_a[$stock_sku] = ["in" => $_in, "out" => $_out, "pname" => $stockupdate['products_name'], "patt" => $attarray[$stockupdate['products_attributes']], "mid" => $stockupdate['manufacturers_id'], "mname"=>$stockupdate['manufacturers_name'], "cid" => $stockupdate['categories_id'], "cname" => $stockupdate['categories_name']];
            } else {
                $stock_sku_a[$stock_sku]["in"] += $_in;
                $stock_sku_a[$stock_sku]["out"] += $_out;
            }
            ...
            if (!isset($stock_sku_a[$stock_sku])) {
                $stock_sku_a[$stock_sku] = ["in" => $_in, "out" => $_out, "pname" => $stockupdate['products_name'], "pv" => $pvarray[$stockupdate['parent_id'] . "-" . $stockupdate['products_id']], "mid" => $stockupdate['manufacturers_id'], "mname"=>$stockupdate['manufacturers_name'], "cid" => $stockupdate['categories_id'], "cname" => $stockupdate['categories_name']];
            } else {
                $stock_sku_a[$stock_sku]["in"] += $_in;
                $stock_sku_a[$stock_sku]["out"] += $_out;
            }
            ...
            $stock_sku = $stockupdate['products_id'];
            if (!isset($stock_sku_a[$stock_sku])) {
                $stock_sku_a[$stock_sku] = ["in" => $_in, "out" => $_out, "pname" => $stockupdate['products_name'], "mid" => $stockupdate['manufacturers_id'], "mname"=>$stockupdate['manufacturers_name'], "cid" => $stockupdate['categories_id'], "cname" => $stockupdate['categories_name']];
            } else {
                $stock_sku_a[$stock_sku]["in"] += $_in;
                $stock_sku_a[$stock_sku]["out"] += $_out;
            }
...
//OSCLogger::writeLog(sizeof($stock_sku_a), OSCLogger::DEBUG);
//OSCLogger::writeLog($stock_sku_a, OSCLogger::DEBUG);
//exit;
/*
    $haircare_checkout = [];
    $hair_checkout = [];
...
*/
    ...
    while ($sales_products = tep_db_fetch_array($sales_products_query)) {
        ...
        if (in_array($sales_products['products_id'], $sample_pid) && (float)$sales_products['final_price'] <1) $sales_products['products_name'] = "Sample " . $sales_products['products_name'];
    ...
    foreach ($opid as $k => $v) {
        if (in_array($v['mid'], $hairextensions)) {
            if (!isset($hair[$v['cid']][$v['sku']])) {
                ...
                if (isset($stock_sku_a[$v['sku']])) {
                    $hair[$v['cid']][$v['sku']]["in"] = $stock_sku_a[$v['sku']]["in"];
                    $hair[$v['cid']][$v['sku']]["out"] = $stock_sku_a[$v['sku']]["out"];
                    unset($stock_sku_a[$v['sku']]);
                }
            }
            else {
            ...
            if (!isset($haircare[$v['mid']][$v['sku']])) {
                ...
                if (isset($stock_sku_a[$v['sku']])) {
                    $haircare[$v['mid']][$v['sku']]["in"] = $stock_sku_a[$v['sku']]["in"];
                    $haircare[$v['mid']][$v['sku']]["out"] = $stock_sku_a[$v['sku']]["out"];
                    unset($stock_sku_a[$v['sku']]);
                }
            } else {
...
//OSCLogger::writeLog($haircare, OSCLogger::DEBUG);
    ...
    $specials_string = "cat, name, att, qty, sale, checkout, checkin" . "\r\n";
    foreach ($hair as $k1 => $v1) {
        $cdesc = tep_db_fetch_array(tep_db_query("SELECT categories_name FROM categories_description WHERE categories_id = " . (int)$k1 . " and language_id=1"));
        $specials_string .= $cdesc['categories_name'] . ", , , , , ," . "\r\n";
        //$cdesc['categories_name'] = preg_replace("/\//", "", $cdesc['categories_name']);
        foreach ($v1 as $k2 => $v2) {
            $specials_string .= " ," . $v2['pname'] . "," . $v2['patt_pv'] . "," . $v2['qty'] . "," . $v2['sale'] . "," . $v2['out'] . "," . $v2['in'] . "\r\n";
        }
    }
    write_to_file($specials_string, "h" . ".csv");
    $specials_string = "brand, name, att, qty, sale, checkout, checkin" . "\r\n";
    foreach ($haircare as $k3 => $v3) {
        $mdesc = tep_db_fetch_array(tep_db_query("SELECT manufacturers_id, manufacturers_name FROM manufacturers where manufacturers_id = " . (int)$k3 . ""));
        if (!tep_not_null($mdesc['manufacturers_name'])) $mdesc['manufacturers_name'] = "unbranded";
        $specials_string .= $mdesc['manufacturers_name'] . ", , , , , ," . "\r\n";
        //$mdesc['manufacturers_name'] = preg_replace("/\//", "", $mdesc['manufacturers_name']);
        foreach ($v3 as $k4 => $v4) {
            $specials_string .= " ," . $v4['pname'] . "," . $v4['patt_pv'] . "," . $v4['qty'] . "," . $v4['sale'] . "," . $v4['out'] . "," . $v4['in'] . "\r\n";
        }
    }
    write_to_file($specials_string, "ha" . ".csv");
...
/*
    ...
    $specials_string = "cat, name, att, qty, out, in" . "\r\n";
    foreach ($hair_checkout as $k1 => $v1) {
        $cdesc = tep_db_fetch_array(tep_db_query("SELECT categories_name FROM categories_description WHERE categories_id = " . (int)$k1 . " and language_id=1"));
        $cdesc['categories_name'] = preg_replace("/\//", "", $cdesc['categories_name']);
        $specials_string = $cdesc['categories_name'] . ", , , , , " . "\r\n";
        foreach ($v1 as $k2 => $v2) {
            $specials_string .= " ," . $v2['pname'] . "," . $v2['patt_pv'] . "," . $v2['qty'] . "," . $v2['out'] . "," . $v2['in'] . "\r\n";
        }
    }
    write_to_file($specials_string, "_" . "h" . ".csv");

    $specials_string = $mdesc['manufacturers_name'] . ", name, att, qty, out, in" . "\r\n";
    foreach ($haircare_checkout as $k3 => $v3) {
        $mdesc = tep_db_fetch_array(tep_db_query("SELECT manufacturers_id, manufacturers_name FROM manufacturers where manufacturers_id = " . (int)$k3 . ""));
        if (!tep_not_null($mdesc['manufacturers_name'])) $mdesc['manufacturers_name'] = "unbranded";
        $mdesc['manufacturers_name'] = preg_replace("/\//", "", $mdesc['manufacturers_name']);
        $specials_string = $mdesc['manufacturers_name'] . ", , , , , " . "\r\n";
        foreach ($v3 as $k4 => $v4) {
            $specials_string .= " ," . $v4['pname'] . "," . $v4['patt_pv'] . "," . $v4['qty'] . "," . $v4['out'] . "," . $v4['in'] . "\r\n";
        }
        write_to_file($specials_string, "_" . "ha" . ".csv");
    }
*/
