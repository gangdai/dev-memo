...
//My modification, check if any order to exclude
    if ($HTTP_POST_VARS['exinvoicenumbers'] != '') {
...
//My modification, check if any order to exclude
    ...
    if ($invoicenumbers != '') {
    	...
    	if ($exinvoicenumbers != '') {
    		...
    		if (preg_match('/refundreport/i', $HTTP_POST_VARS['file_type'])) {
    			$orders_query = tep_db_query("select ot.orders_total_id, ot.orders_id, ot.title, ot.text, ot.value, ot.class, osh.date_added, osh.orders_returned_reasons_id, o.payment_method, o.billing_country, o.billing_name from " . TABLE_ORDERS_TOTAL . " ot left join " . TABLE_ORDERS_STATUS_HISTORY . " osh on ot.orders_total_id=osh.orders_total_id left join orders o on ot.orders_id=o.orders_id where osh.date_added between '" . tep_db_input($startdate) . " 00:00:00' and '" . tep_db_input($enddate) . " 23:59:59' and ot.class = 'ot_refund' and osh.orders_total_id is not null and o.cash_carry=0");
    		}
    		else {
...
//My modification
    ...
    if (preg_match('/interlink/i', $HTTP_POST_VARS['file_type']) || preg_match('/ups/i', $HTTP_POST_VARS['file_type']) || preg_match('/rmtrack/i', $HTTP_POST_VARS['file_type']) || preg_match('/lowcostparcel/i', $HTTP_POST_VARS['file_type']) || preg_match('/ekomi/i', $HTTP_POST_VARS['file_type']) || preg_match('/refundreport/i', $HTTP_POST_VARS['file_type']) || preg_match('/ukmail/i', $HTTP_POST_VARS['file_type']) || preg_match('/sagepayhr/i', $HTTP_POST_VARS['file_type'])) { exit; }
...
function print_ekomi_feedback($order) {
  global $startdate,$enddate; //global $HTTP_POST_VARS;
  global $delimiter;
  global $orders; //$orders['orders_id']
