...
//echo "<br /><br /><br /><br />" . print_r($HTTP_POST_FILES);exit;
        //for ($i=1; $i<6; $i++) {
          if (is_uploaded_file($HTTP_POST_FILES['file_']['tmp_name'])) {
            ...
            elseif ($HTTP_POST_VARS['filetype'] ==5) { //amazon update stock - using the daily order report file (unshipped) from sellercentral
                  $handler = opendir($current_path);
                  // open directory and walk through the filenames
                  while ($file = readdir($handler)) {
                    if ($file != "." && $file != "..") {
                      if ($file == $HTTP_POST_FILES['file_']['name']) {
                        ...
                        while (!feof($file_handle)) {
                          ...
                          $splitcontents = str_getcsv($line_of_text, "\t", '"');
                          if (sizeof($splitcontents)>1 && sizeof($splitcontents) == 24) { //amazon export 24
                          	//$splitcontents[10] - sku, $splitcontents[12] - quantity-purchased
                          	if (0 == preg_match('/[^\x00-\x7f]/', $splitcontents[10])) {} //check if sku contain non-ascii char
                          	else { $splitcontents[10] = preg_replace("/[\x80-\xff]/", "-", $splitcontents[10]); } //replace non-ascii char (or extended ascii char by hyphen)
                          	$sku_array[$splitcontents[10]] += (int)$splitcontents[12]; 
                          }
                          else { //ebay export 48
                          }
                        }
                        fclose($file_handle);
                        if (sizeof($sku_array)) {
                        	if (file_exists('includes/amazon_sku_dict.php')) { //check if $amazon_sku_dict exist
                            require('includes/amazon_sku_dict.php');
                            if (isset($amazon_sku_dict) && sizeof($amazon_sku_dict)) {
                        	    foreach ($sku_array as $k => $v) {
                        	  	  if (array_key_exists($k, $amazon_sku_dict)) { //$k is the sku from amazon sellercentral, $amazon_sku_dict[$k] is the according live sku
                        	  	  	$entry = update_stock($amazon_sku_dict[$k], $v, IKSH_COMMENTS_CHECKOUT, shop_amazon_id());
                        	  	  }
                        	  	  else { //if amazon sku does not exist in the $amazon_sku_dict cache
                        	  	  	$entry = update_stock($k, $v, IKSH_COMMENTS_CHECKOUT, shop_amazon_id());
                        	  	  }
                        	    }
                        	  }
                          }
...
<?php
    ...
    switch ($action) {
      ...
      case 'upload':
        ...
        //20151026
        $filetype = array(array('id' => 1, 'text' => TEXT_UPLOAD_ROYALMAIL), array('id' => 2, 'text' => TEXT_UPLOAD_RM), array('id' => 3, 'text' => TEXT_UPLOAD_LCP), array('id' => 4, 'text' => TEXT_AMAZON_SHIPPING_CONFIRMATION), array('id' => 5, 'text' => TEXT_AMAZON_STOCK_UPDATE));
